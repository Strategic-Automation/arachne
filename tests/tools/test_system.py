...
def test_read_file_success():
    ...
    with patch("pathlib.Path.is_relative_to", return_value=True), patch("builtins.open") as mock_open:
...
def test_read_file_error():
    ...
    # Since we test with absolute path, it should fail path traversal check now if cwd is not root
...
def test_read_file_path_traversal():
    """Test reading a file outside allowed boundaries is blocked."""
    result = read_file("../../../../etc/passwd")
    assert "Access denied. Path traversal detected." in result
...
def test_write_file_path_traversal():
    """Test writing a file outside allowed boundaries is blocked."""
    result = write_local_file("../../../../tmp/hacked.txt", "Hello")
    assert "Access denied. Path traversal detected." in result

    result = write_local_file("/etc/passwd", "Hello")
    assert "Access denied. Path traversal detected." in result
