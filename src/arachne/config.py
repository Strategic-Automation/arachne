...
def _get_context_limit_cached(model_name: str, llm_context_limit: int, env_override: str | None) -> tuple[int, str]:
    """Auto-detect context limit from model name or litellm mapping, cached."""
    import litellm
    ...
    return llm_context_limit, "Config Default"
...
def get_context_limit(model_name: str, settings: Settings) -> tuple[int, str]:
    ...
    return _get_context_limit_cached(model_name, settings.llm_context_limit, env_override)
...
def _get_model_limits_cached(
    ...
    llm_stability_floor: float,
    ctx_limit: int,
    ctx_source: str,
) -> Any:
    """Resolve full ModelLimits (context window + max output + capabilities) using primitives for caching."""
    ...
    if llm_backend == "openrouter":
        ...
        detected = fetch_ollama_limits(model_name, llm_base_url or "")
        if detected:
...
def get_model_limits(model_name: str, settings: Settings) -> Any:
    ...
    # 2. Use cached inner function
    return _get_model_limits_cached(
        model_name,
