...
def haversine_m(lat1, lon1, lat2, lon2):
    ...
    radius = 6_371_000  # Earth mean radius in metres
...
def build_overpass_nearby(tag_key, tag_val, lat, lon, radius, limit, religion=None):
...
def build_overpass_bbox(tag_key, tag_val, south, west, north, east, limit, religion=None):
...
def cmd_nearby(args):
    ...
    for category in categories:
        ...
        query = build_overpass_nearby(tag_key, tag_val, lat, lon, radius, limit, religion=religion)
...
def cmd_timezone(args):
    ...
    if not timezone_str:
        ...
        utc_offset = f"+{approx_offset_h:02d}:00" if approx_offset_h >= 0 else f"-{abs(approx_offset_h):02d}:00"
