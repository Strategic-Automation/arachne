...
def list_graphs() -> None:
    ...
    if not cache_dir.exists():
        console.print(
            "[dim]No cached graphs found. Run a goal with [bold white]arachne run[/bold white] first to generate topologies.[/dim]"
        )
        return
...
def cat_session(
    ...
    base = default_session_dir()
    if session_id == "last":
        sessions = sorted(base.iterdir(), key=lambda p: p.stat().st_mtime, reverse=True) if base.exists() else []
        if not sessions:
            console.print(
